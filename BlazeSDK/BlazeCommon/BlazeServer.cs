    ...
    ProtoFirePacket packet)
        {
            ...
            {
                var pongFrame = new FireFrame
                {
                    Component = frame.Component,
                    Command = frame.Command,
                    MsgNum = frame.MsgNum,
                    UserIndex = frame.UserIndex,
                    MsgType = FireFrame.MessageType.PING_REPLY
                };

                var pongPacket = new ProtoFirePacket(
                    pongFrame,
                    Array.Empty<byte>(),
                    packet.Framing
                );

                await connection.SendAsync(pongPacket)
                    .ConfigureAwait(false);

                _logger.Debug(
                    "Connection({ClientId}) replied to Fire2 ping ID[{MessageId}].",
                    connection.ID,
                    frame.MsgNum
                );
            ...
            if (frame.MsgType == FireFrame.MessageType.PING_REPLY)
                return;
